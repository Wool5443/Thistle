...
{
    String tree_string = TRY_RES(string_ctor_capacity(thistle_arena_resource, 1));
    TRY(read_file(&tree_string, input_path));
    UNUSED Node* tree = tree_read(str_ctor_string(tree_string));

    Symbol_table symbols = {};

    symbol_add(&symbols, TRY_RES(string_ctor(thistle_arena_resource, "print")));
